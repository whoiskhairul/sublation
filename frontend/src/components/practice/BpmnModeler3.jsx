  ...
  const handleErrorDetection = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://127.0.0.1:8000/bpmn_error_detection/validate/",
        formData,
        {
...
const showDiagramWarnings = () => {
  ...
  const [optimizationScopes, setOptimizationScopes] = useState([]); // State to store optimization scopes

  const handleOptimizationScopeDetection = async () => {
    if (!modelerRef.current) return;
    ...
    try {
        ...
        const formData = new FormData();
        formData.append("file", new Blob([xml], { type: "text/xml" }), "diagram.bpmn");

        const response = await axios.post(
            "http://127.0.0.1:8000/bpmn-optimization/scope/",
            formData
        );

        if (response.status === 200) {
            const { optimizationSuggestions, message } = response.data;

            console.log("Response Data:", response.data);
            console.log(message);

            // Safeguard against undefined or empty responses
            if (optimizationSuggestions && Array.isArray(optimizationSuggestions)) {
                optimizationSuggestions.forEach((suggestion) => {
                    console.log("Optimization Suggestion:", suggestion);
                });
            } else {
                console.warn("No optimization suggestions found in response.");
            }
        } else {
        ...
        console.error(
            "Error detecting optimization scopes:",
            error.response?.data || error.message
        );
    }
};

const handleApplyOptimization = async (elementId) => {
    if (!modelerRef.current) return;
    ...
    try {
        const response = await axios.post(
            "http://127.0.0.1:8000/bpmn-optimization/apply/",
            formData
        );

        if (response.status === 200) {
            const optimizedXml = response.data.optimizedXml;
            await modelerRef.current.importXML(optimizedXml);
